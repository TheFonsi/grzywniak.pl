...
function grantAgentRepositoryAccess(string $owner,string $repository,string $token): void {
    $org=trim(projectSetting('GITHUB_ORG'));
    $team=trim(projectSetting('GITHUB_AGENT_TEAM_SLUG'));
    if($team!=='') {
        if(!preg_match('/^[A-Za-z0-9-]+$/',$team)) throw new RuntimeException('Niepoprawna nazwa zespołu GitHub agentów.');
        $access=githubApi('PUT','/orgs/'.rawurlencode($org).'/teams/'.rawurlencode($team).'/repos/'.rawurlencode($owner).'/'.rawurlencode($repository),['permission'=>'push'],$token);
        if($access['status']!==204) throw new RuntimeException('Nie udało się nadać zespołowi agentów dostępu do repozytorium (HTTP '.$access['status'].').');
        return;
    }
    ...
    $access=githubApi('PUT','/repos/'.rawurlencode($owner).'/'.rawurlencode($repository).'/collaborators/'.rawurlencode($agent),['permission'=>'push'],$token);
